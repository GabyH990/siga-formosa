<x-app-interno-layout>
    @section('title', 'Armar Cursada – SIGA')
                ...
                <form method="GET" action="{{ route('asistencias.armar-cursada') }}" class="space-y-4">
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div>
                            ...
                            <select name="subject_id" onchange="this.form.submit()"
                                class="w-full rounded-md border-violet-300 shadow-sm focus:border-violet-500 focus:ring-violet-500 dark:bg-indigo-300 dark:border-violet-600 dark:text-black">
                        ...
                        <div>
                            ...
                            <select name="commission_nombre" onchange="this.form.submit()"
                                ...
                                @foreach ($commissionOptions as $opt)
                                    <option value="{{ $opt }}" {{ (string) $commissionNombre === (string) $opt ? 'selected' : '' }}>
                                        Comisión {{ $opt }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select name="professor_id"
                                ...
                                @foreach ($professors as $prof)
                                    <option value="{{ $prof->id }}" {{ (string) $professorId === (string) $prof->id ? 'selected' : '' }}>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                        <div class="md:col-span-2">
                            ...
                            <input type="text" name="filter" value="{{ $filter }}" placeholder="Buscar alumno…"
                                class="w-full rounded-md border-violet-300 shadow-sm focus:border-violet-500 focus:ring-violet-500 dark:bg-indigo-300 dark:border-violet-600 dark:text-black">
                        </div>
        ...
        @if ($subjectId && $commissionNombre)
            <form method="POST" action="{{ route('asistencias.armar-cursada.guardar') }}" class="space-y-4">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    {{-- ==== ELEGIBLES ==== --}}
                    <div class="p-[1px] rounded-lg bg-gradient-to-r from-[#ca98f5] to-[#6daff1] ">
                        <div class="bg-white dark:bg-violet-950 shadow rounded-lg p-4 flex flex-col">
                            ...
                            @else
                                <div class="flex-1 overflow-auto border rounded-md">
                                    <table class="min-w-full text-sm">
                                        <thead class="bg-violet-200 dark:bg-purple-950">
                                            <tr>
                                                ...
                                                <th class="px-3 py-2 dark:text-white text-left">Legajo</th>
                                                <th class="px-3 py-2 dark:text-white text-left">Apellido y Nombre</th>
                                            </tr>
                                        </thead>
                                        <tbody class=" dark:bg-purple-800 divide-y divide-purple-200">
                                            @foreach ($elegibles as $alumno)
                                                <tr>
                                                    <td class="px-3 py-1">
                                                        <input type="checkbox" name="add_students[]" value="{{ $alumno->id }}">
                                                    </td>
                                                    <td class="px-3 py-1 text-sm text-gray-900 dark:text-white">
                                                        {{ $alumno->legajo }}
                                                    </td>
                                                    <td class="px-3 py-1 text-sm text-gray-900 dark:text-white">
                            ...
                            @else
                                <div class="flex-1 overflow-auto border rounded-md">
                                    <table class="min-w-full text-sm">
                                        ...
                                        <tbody class="dark:bg-purple-800 divide-y divide-violet-200">
                ...
                <div class="flex justify-end">
                    <button type="submit"
                        class="inline-flex items-center px-6 py-2 bg-indigo-300 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-700 focus:bg-indigo-700 active:bg-indigo-800 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">
                        Guardar cursada
                    </button>
                </div>
            </form>
